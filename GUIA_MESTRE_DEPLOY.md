...
2.  **Crie o Cluster:** Clique em "Create" e escolha a opção **M0 (Free)**. Escolha qualquer servidor (AWS/GCP) e clique em "Create Deployment".
3.  **Segurança (Usuário):** Ele vai pedir para criar um usuário. 
...
4.  **Segurança (IP):** Em "IP Access List", clique em "Add My Current IP Address" e também adicione o IP `0.0.0.0/0` (isso permite que a Vercel acesse o banco). Clique em "Finish and Close".
5.  **Pegar o Link de Conexão:**
...
2.  No painel da Vercel, clique no seu projeto.
3.  Vá na aba **"Settings"** (no topo) e depois em **"Environment Variables"** (na lateral esquerda).
4.  Adicione a variável que faz o banco funcionar:
    - **Key:** `MONGO_URI`
    - **Value:** Cole aquele link completo do MongoDB que você preparou no passo anterior.
    - Clique em **"Add"**.
5.  **DICA PARA O DESIGN:** Vá na aba **"Deployments"**, clique nos três pontinhos do seu último deploy e escolha **"Redeploy"**. Isso vai forçar a Vercel a ler as novas configurações de pastas que eu fiz para o CSS não sumir.
...
1. [ ] Criei o banco no MongoDB.
2. [ ] Coloquei o link `MONGO_URI` nas "Environment Variables" da Vercel.
3. [ ] Fiz o "Redeploy" na Vercel.
4. [ ] Cadastrei meu e-mail `jhonybrandoborges@gmail.com` no site.
5. [ ] Vi o botão "Painel Admin" aparecer.
